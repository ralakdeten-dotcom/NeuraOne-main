...
class CallListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = Call.objects.filter(is_active=True).select_related(
        ...
        if entity_type and entity_id:
            ...
            # Standalone calls (not linked to any entity)
        ...
        # Default ordering: most recent calls first
        return queryset.order_by('-call_date', '-call_time', '-created_at')
...
class CallDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        return Call.objects.filter(is_active=True).select_related(
            'created_by', 'content_type', 'contact'
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            ...
            if instance.created_by != request.user:
                ...
                raise PermissionDenied("You can only delete calls you created.")
            ...
            return Response(
                {'error': f'Failed to delete call: {str(e)}'},
...
class MyCallsView(generics.ListAPIView):
    """
    List calls created by the current user.
    """
...
class EntityCallsView(generics.ListCreateAPIView):
    """
    List calls for a specific entity or create a new call linked to an entity.
    URL pattern: /calls/{entity_type}/{entity_id}/
    """
    permission_classes = [permissions.IsAuthenticated, IsTenantUser, HasTenantPermission]
    required_permissions = ['all', 'view_customers', 'manage_opportunities']
    ...
    def get_queryset(self):
        ...
        try:
            ...
            return Call.objects.filter(
                ...
                object_id=entity_id,
                is_active=True
            ).select_related('created_by', 'contact').order_by('-call_date', '-call_time')
        except ContentType.DoesNotExist:
...
class ContactCallsView(generics.ListAPIView):
    """
    List calls for a specific contact.
    URL pattern: /calls/contact/{contact_id}/
    """
    serializer_class = CallListSerializer
    permission_classes = [permissions.IsAuthenticated, IsTenantUser, HasTenantPermission]
    required_permissions = ['all', 'view_customers', 'manage_opportunities']
    ...
    def get_queryset(self):
        ...
        return Call.objects.filter(
            ...
            is_active=True
        ).select_related('created_by').order_by('-call_date', '-call_time')


class CallCommentListCreateView(generics.ListCreateAPIView):
    """
    List comments for a call or create a new comment.
    """
    ...
    def perform_create(self, serializer):
        call_id = self.kwargs.get('call_id')
        serializer.save(author=self.request.user, call_id=call_id)


class CallCommentDetailView(generics.RetrieveUpdateDestroyAPIView):
    """
    Retrieve, update or delete a call comment.
    """
    serializer_class = CallCommentSerializer
    permission_classes = [permissions.IsAuthenticated, IsTenantUser]

    def get_queryset(self):
        return CallComment.objects.filter(is_active=True).select_related('author')
...
def call_stats(request):
    """
    Get call statistics for the current user and overall.
    """
    user = request.user
    now = timezone.now()

    # Base queryset for active calls
    base_qs = Call.objects.filter(is_active=True)

    # Calculate statistics
    stats = {
        'total_calls': base_qs.count(),
        'inbound_calls': base_qs.filter(direction='inbound').count(),
        'outbound_calls': base_qs.filter(direction='outbound').count(),
        'completed_calls': base_qs.filter(status='completed').count(),
        'missed_calls': base_qs.filter(status='missed').count(),
        'calls_with_followup': base_qs.filter(follow_up_required=True).count(),
        'overdue_followups': base_qs.filter(
            follow_up_required=True,
            follow_up_date__lt=now
        ).count(),
        'my_calls': base_qs.filter(created_by=user).count(),
...
def update_call_status(request, pk):
    ...
    """
    try:
        call = Call.objects.get(pk=pk, is_active=True)
        ...
        if new_status not in dict(Call.STATUS_CHOICES):
            return Response(
